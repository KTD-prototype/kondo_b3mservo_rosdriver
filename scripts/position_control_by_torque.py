...
def initial_process():
    global id, num
    global target_position_by_torque, voltage
    ...
    target_position_by_torque = list(target_position_by_torque)
    the_number_of_servo_pub.publish(num)


def callback_servo_command(multi_servo_command):
    global num, target_position_by_torque, id, merged_command
    ...
    if servo_drive_flag == 1:
        ...
        target_position_by_torque = list(target_position_by_torque)
        for i in range(num):
            Drive.control_servo_position_by_Torque(
                id[i], target_position_by_torque[i])
        publish_servo_info()
        merged_command = []
